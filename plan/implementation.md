...
- **Projects view: tracking large tasks & delegated work** (M) — https://github.com/sunix/todowai/issues/25 — *Merged (#94).*
  Build the Projects screen: cards for large tasks, parallel work, and AI-delegated work, with status badges and progress indicators. Scans notes for `type: project` frontmatter; read-only (editing happens via Notebook). Real usage immediately surfaced follow-ups, tracked separately (not part of the original Phase 3 plan): [#95](https://github.com/sunix/todowai/issues/95) (task checklists with auto-derived progress, merged), [#99](https://github.com/sunix/todowai/issues/99) (AI-proposed attachment to an existing note instead of always creating a new one, merged via #100 — supersedes the earlier manual-picker design in #96, closed unmerged), and [#101](https://github.com/sunix/todowai/issues/101) (AI proposing several actions from one capture at once, e.g. checking off a task while also adding a new one, merged via #102).
